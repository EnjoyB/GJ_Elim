...
//            for (int i = 0; i < 2; ++i) {
        ...
        t1 = high_resolution_clock::now(); // Begins timeout
        ...
        delete[] matrix;
        t2 = high_resolution_clock::now(); // Ends timeou
t        time_span = duration_cast<duration<double>>(t2 - t1); // Result time
        myResultsFile << length << "\t" << time_span.count() << endl;
        // }
...
//            for (int i = 0; i < 2; ++i) {
            ...
            delete[] matrix;
            t2 = high_resolution_clock::now(); // Ends timeout
            time_span = duration_cast<duration<double>>(t2 - t1); // Result time
            myResultsFile << length << "\t" << time_span.count() << endl;
        }
...
//        printMatrix(&matrix, &length);
        cout << "Procces duration: " << time_span.count() << " seconds " << endl;
    }
...
bool operateMatrixFirst(float ***matrix, int length) {
    ...
    for (int i = 0; i < length; ++i) {
        ...
        */
        ...
        // Prechadzam X-sovu os, but only to foreLast item
        for (int y = i + 1; y < length*2; ++y) {


            float multi = (*matrix)[y][i] / (*matrix)[i][i];
            // cout << " Multi is: " << multi << endl;

            // Prechadzam Y-lonovu os
